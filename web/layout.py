...
# Mobile tab bar labels (shorter, fit 5 tabs)
_MOB_TAB_LABELS = {
    "dashboard":  "Hoy",
    "projects":   "Proyectos",
    "calendar":   "Agenda",
...
# Mobile tab bar items: (key, label, url_key, show_badge_id)
_MOB_TAB_ITEMS = [
    ("dashboard",  "Hoy",       "/",           None),
    ("projects",   "Proyectos", "/projects",   "mob-badge-proj"),
    ("calendar",   "Agenda",    "/calendar",   None),
    ("inventory",  "Stock",     "/inventory",  "mob-badge-stock"),
    ("users",      "Más",       "/users",      None),
]
...
def _shell(page, user, content, extra_head="", title=""):
    ...
    _mob_tab_icons = {
        "dashboard": '<svg viewBox="0 0 24 24"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>',
        ...
        "users":     '<svg viewBox="0 0 24 24"><circle cx="5" cy="12" r="1"/><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/></svg>',
    }
    mob_tabs_html = ""
    for key, label, url_path, badge_id in _MOB_TAB_ITEMS:
        active_cls = " active" if page == key else ""
        url = bp + url_path
        icon_svg = _mob_tab_icons.get(key, "")
        badge_html = f'<span class="mob-badge" id="{badge_id}" style="display:none"></span>' if badge_id else ""
        mob_tabs_html += (
            f'<a href="{url}" class="mob-tab{active_cls}" aria-label="{label}">'
            f'{icon_svg}{badge_html}'
            f'<span>{label}</span>'
            f'</a>\n'
        )
...
<nav class="mob-tabs" aria-label="Navegación móvil">
{mob_tabs_html}</nav>

<!-- Mobile FAB: nuevo proyecto -->
<a href="{bp}/projects?new=proyecto" class="mob-fab" aria-label="Nuevo proyecto"
   onclick="if(typeof openNewProject==='function'){{event.preventDefault();openNewProject();}}">
